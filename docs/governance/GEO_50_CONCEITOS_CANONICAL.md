...
> **Aplicação em geo-ipog:** estes 50 conceitos são a TAXONOMIA EDITORIAL canônica para toda página criada no site (HowTo, FAQ, comparativo, guia clínico, vertical de modalidade). Sub-agents Opus que produzem conteúdo recebem este arquivo como contexto obrigatório. Para vertical psicologia: conceito 9 (intenção) considera busca por aluno, profissional CFP, gestor de instituição e familiar de paciente — quatro intenções distintas.
...
## Aplicação canônica por vertical do geo-ipog

Mapa de uso obrigatório para cada tipo de página deste portal. Sub-agents Opus DEVEM consultar este bloco antes de iniciar produção.

### Páginas HowTo (Wave AA — guias práticos)
Prioridade: **7, 8, 11, 13, 25**.
- Conceito 7 (hierarquia H2/H3) define a estrutura dos passos.
- Conceito 11 (answer capsules) garante extração por LLM.
- Conceito 13 exige JSON-LD `HowTo` com `step` array coerente com o visível (Conceito 14).
- Conceito 25 (recuperabilidade) demanda referências e atualidade.

### Páginas FAQ (Waves BB, FF, KK — perguntas profundas)
Prioridade: **11, 12, 13, 14**.
- Conceito 12 exige perguntas reais (extraídas de Reddit/People Also Ask/Perplexity), não fabricadas.
- Conceito 13 marca `FAQPage` + Speakable. Atenção: FAQ rich result foi descontinuado em 07/05/2026 (ver KB 17/05), mas o schema permanece útil para LLMs.
- Conceito 14 bloqueia FAQ inflado: toda Q&A do JSON-LD deve existir no HTML visível.

### Páginas comparativas (Wave CC — A vs B)
Prioridade: **9, 10, 49**.
- Conceito 9 trata intenção comparativa explícita (ex.: "Lato Sensu vs Mestrado Profissional").
- Conceito 10 garante cobertura semântica das duas pontas (carga horária, CFP, custo, ROI, perfil).
- Conceito 49 (concorrência) obriga benchmarking com fontes reais, sem afirmar superioridade sem evidência.

### Páginas clínicas (Waves HH, JJ, MM — saúde mental, neuropsi, IA clínica)
Prioridade: **21, 22, 23 — E-E-A-T crítico em saúde mental**.
- Conceito 21 exige referências externas verificáveis (CFP, ANVISA, papers DOI ativo, RDC, Portaria, leis).
- Conceito 22 obriga autoria explícita (Alexandre Caramaschi como editor + autor técnico quando aplicável + revisão clínica quando disponível).
- Conceito 23 (E-E-A-T) é o gate mais estrito do portal: páginas clínicas SEM evidência verificável devem ser despublicadas.
- Spot-check obrigatório de 3-5 citações antes do build (regra já canônica em CLAUDE.md raiz).

### Páginas de modalidade (5 verticais canônicas — Lato Sensu, MBA, Mestrado Profissional, Clínica certificada, Híbridas)
Prioridade: **9 + 10 + 15**.
- Conceito 9 distingue as **quatro intenções por buscador**: aluno em decisão, profissional CFP buscando atualização, gestor de instituição parceira, familiar de paciente buscando entender credenciais.
- Conceito 10 cobre subtópicos da modalidade (carga, certificação MEC/CFP, formato, ROI, mercado).
- Conceito 15 (clareza de entidade) reforça o reconhecimento canônico de IPOG e Brasil GEO via JSON-LD.

### Anti-padrão 26 — veto absoluto no portal de psicologia
Banido em qualquer copy do site (página, FAQ, comparativo, HowTo):
- "garante CFP", "aprovado CFP garantido" (CFP não certifica pós; apenas o MEC/instituições credenciadas).
- "aprovado garantido", "100% de aprovação".
- "carreira em 30 dias", "carreira em 90 dias", "vire psicólogo em X tempo".
- "psicólogo top 1", "especialista #1", "única pós que ensina X".
- "ChatGPT já cita", "AI Overview garantido", "Perplexity reconhece" (pseudo-GEO).

### Auditoria técnica recorrente
Prioridade canônica: **1, 2, 13, 14, 28, 29, 30, 34, 35, 36, 37**.
- Conceitos 28-30 são gates de build (robots.txt, sitemap.xml, llms.txt validados a cada deploy).
- Conceitos 34-37 cobertos por Lighthouse CI (`lighthouserc.json` na raiz).
- Conceitos 13-14 cobertos pelo `audit-graph-triple-2026-05-17.md`.

### Onboarding de sub-agents Opus
Todo prompt de spawn de sub-agent que produzir página/FAQ/comparativo neste repo DEVE incluir:
1. Este arquivo (`docs/governance/GEO_50_CONCEITOS_CANONICAL.md`) como contexto.
2. A lista de prioridades por tipo de página (bloco acima).
3. Anti-padrão 26 (veto absoluto) literal no prompt.
4. Regra PT-BR com acentuação completa + slugs ASCII (já canônica em CLAUDE.md raiz).
