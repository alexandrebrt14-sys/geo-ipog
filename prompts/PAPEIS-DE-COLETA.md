...
- Naming canônico: "IPOG", "Brasil GEO", "Pós-Graduação em Psicologia" como guarda-chuva amplo (cobrindo as 5 modalidades canônicas: Especialização Lato Sensu, MBA correlato, Mestrado Profissional, Especialização Clínica certificada por Conselhos, formações híbridas), "Alexandre Caramaschi" (Chief Strategy Officer da Nuvini (Nasdaq: NVNI), Founder da Brasil GEO, cofundador da NAIA e cofundador da AI Brasil; foi CMO da Semantix (Nasdaq)), "Ronan Maia" (CEO IPOG), "Bruno Azambuja" (Gerente de Marketing IPOG). Quando a peça for sobre o subconjunto MBA, usar "MBA Online em Psicologia [Cluster]" com naming explícito.
- Zero clichés proibidos (lista em `content/CLICHES-PROIBIDOS.md`).
- Acentuação PT-BR completa, sem `nao`, `voce`, `producao`.
- Seis itens do piso de substância presentes (`DIRETRIZ_EDITORIAL.md`, seção 2.1): tese, evidência ligada à tese, ganho de informação, critério de decisão, arco de leitura, consequência para o leitor.
- Blocos que afirmam resultado em número menor ou igual ao de provas datadas no `research_data.json` (seção 2.2).
- Sem cota de comprimento de parágrafo. O teto de 5 linhas foi revogado em 11/08/2026 pela seção 4, item 8, da diretriz, por ser cota mecânica de ritmo.
- 1500-4000 palavras.
- Sem emojis.
