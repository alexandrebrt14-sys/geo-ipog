...
Este repositório executa o programa GEO do IPOG: mede a presença das Pós-Graduações em Psicologia do IPOG nas respostas de motores generativos e produz o conteúdo editorial que sustenta essa presença, publicado no portal independente `posgraduacaopsicologia.com` e nas propriedades do cliente. Quem lê o texto que nasce aqui é o psicólogo em decisão de carreira, o profissional de RH sem formação em Psicologia, o gestor de instituição e o familiar de paciente, além do executivo do IPOG que recebe relatório e do motor generativo que recorta a passagem para citar.

O gênero de cada tipo de peça está fixado na seção 14, junto com as restrições que este domínio impõe: o portão da promessa educacional, a nomenclatura das cinco modalidades canônicas e o tratamento da prova quando o dado pertence ao cliente. Peça editorial assinada e página de guia seguem o gênero "artigo ou relatório" da seção 3.2; página de área, de modalidade e de MBA segue o gênero "página comercial"; exercício, roteiro e material de capacitação seguem o gênero "aula ou material de capacitação".

## Migração da versão 2 (23 de julho de 2026) para a versão 4

Quem trabalha neste repositório memorizou regras da v2 que agora estão revogadas, e a de maior impacto é a cota de amplitude de ritmo. A seção 3 da v2 prescrevia que, num bloco de dez frases, a diferença entre a mais longa e a mais curta "deve passar de 30 palavras", redigida como alvo a perseguir durante a escrita. A v4 mantém o número como diagnóstico do texto pronto (seção 4, item 1) e proíbe qualquer cota mecânica de cadência na produção (seção 4, item 8), porque a combinação daquelas regras era aritmeticamente insatisfazível e produzia o staccato de manchete que elas diziam combater. O anexo prático refaz a conta na seção 2.

Quatro acréscimos mudam o que se aprova. O piso de substância (seção 2.1) lista os seis itens que toda peça precisa ter, porque doutrina construída só de proibição aprova texto curto e vazio. A prova antes da escrita (seção 2.2) manda levantar a evidência antes da primeira frase e usa essa lista para limitar o tamanho da peça: o número de blocos que afirmam resultado fica menor ou igual ao número de provas datadas disponíveis. A seção 3 inteira, sobre narrativa e promessa, não existia, e traz o portão da promessa (3.1), o esqueleto por gênero (3.2), o caso condutor com rótulo de tipo (3.4), a tensão sem escassez fabricada (3.5) e o pedido único por peça (3.6). A seção 13 fecha com o fluxo de revisão em três passadas e com as quatro conferências que todo símbolo de porcentagem dispara.

A numeração mudou junto. As seções 3 a 12 da v2 correspondem hoje às seções 4 a 13, então referência antiga a "seção 3, humanização" aponta agora para a seção 4, e "seção 12, revisão" aponta para a seção 13. Confira o número antes de citar, porque identificador errado se propaga sozinho para os derivados.

Nada da v2 foi descartado. A comparação byte a byte entre a cópia local e a cópia canônica de julho não achou regra local acrescentada depois da propagação original, o que significa que a substituição por esta versão não apagou trabalho de ninguém. As regras específicas do IPOG sempre viveram fora destes dois arquivos, em `content/`, `prompts/` e `CONTRIBUTING.md`, e a seção 14 diz quais delas continuam valendo e qual delas a v4 revoga.
...
## 14. Aplicação ao escopo deste repositório

### 14.1 O que nasce aqui e sob qual gênero

O gênero de cada peça se decide antes da primeira linha, porque ele fixa a ordem dos blocos e essa ordem não se conserta na revisão sem reescrever tudo. A tabela abaixo enquadra os tipos que este repositório produz.

| Tipo de peça | Onde mora | Gênero (seção 3.2) | Ordem de blocos |
|---|---|---|---|
| Peça editorial assinada, padrão HBR | `content/pecas-hbr/` | Artigo ou relatório | Situação, tensão, tese, evidência, comparação de alternativas, limitações, callback, próximo passo |
| Guia, evidência, caso, comparativo e FAQ do portal | `site/src/pages/{guias,evidencias,casos,comparativos,faq}/` | Artigo ou relatório, com a cápsula de resposta da seção 9 abrindo cada seção interna | Igual ao anterior, com heading que é pergunta real do público |
| Página de área, de modalidade, de MBA, por estado e por persona | `site/src/pages/{areas,mbas,por-estado,para-quem}/` | Página comercial | Situação reconhecível, progresso desejado, mecanismo, prova verificável, objeção, condição de entrada, próximo passo reversível |
| Exercício, roteiro e material de capacitação | `content/EXERCICIO-TEMPLATE.md` e os blocos de exercício das peças | Aula ou material de capacitação | Problema como situação com custo, âncora na experiência do aluno, conceito nomeado, exemplo resolvido inteiro, erros comuns, checklist, modelo, síntese prática |
| Auditoria, síntese executiva e relatório de dashboard | `audits/`, `dashboards/`, `docs/research/` | Artigo ou relatório | Conclusão antes da sustentação, evidência datada, limitação declarada, recomendação com critério |
| Página de wiki, `wiki/log.md` e `STATUS.md` | `wiki/`, raiz | Fora dos gêneros narrativos | Ver o parágrafo seguinte |
| Prompt de LLM | `prompts/` | Instrução de máquina | Ver a subseção 14.6 |

A wiki e o ledger de status ficam de fora do arco da seção 3 por um motivo de unidade de leitura: ali a unidade recuperada é o fato isolado, e um agente que abre `wiki/entities/` quer o dado com a data e a fonte, não uma cena de abertura. Continuam valendo nesses arquivos o piso de substância da seção 2.1, a atribuição nomeada da seção 2, a tipografia da seção 1 e o veto a dado inventado. Aplicar caso condutor a nó de grafo interno produz ruído e atrasa a consulta que a wiki existe para acelerar.

### 14.2 Promessa educacional passa pelo portão da seção 3.1

Nenhuma promessa sobre o que o aluno ganha entra em página, peça ou material deste repositório antes de existirem as três coisas da seção 3.1: uma experiência que o aluno reconhece, uma medida que a representa e uma rota de reparação quando ela falha. "Aulas ao vivo com professores atuantes" só é publicável quando existem o encontro síncrono com professor identificado, a contagem que o descreve (quantas horas síncronas por módulo, com que frequência, com quantos alunos por turma) e a resposta ao que acontece quando o aluno perde a aula ou quando a turma não abre. Faltando a terceira, o texto está vendendo intenção, e o aluno descobre isso depois de pagar.

Promessa de resultado de carreira exige evidência com denominador, e quase nenhuma tem. "Egressos dobram o faturamento" precisa de origem, data, método e denominador dentro da mesma frase: quantos responderam, de quantos formados, em qual janela, medido por quem. Sem os quatro, o número sai e a afirmação encolhe até o tamanho do que se sabe. A mesma trava alcança a família de promessas que o anti-padrão 26 do `CLAUDE.md` já bania ("garante CFP", "aprovado garantido", "carreira em 30 dias"), e alcança a escassez fabricada da seção 3.5, que em captação de pós-graduação costuma aparecer como "últimas vagas", "turma fechando" e "matrículas encerram sexta". A tensão legítima aqui aponta para custo que já corre: o psicólogo que adia a especialização clínica continua recusando encaminhamento que não sabe atender.

Afirmação regulatória tem dono, e não é o autor do texto. Autorização MEC, reconhecimento de curso, carga horária regulamentar, validade nacional do certificado e certificação por Conselho (CFP, ABRAP, FBT, ABPp) passam pela validação jurídica e acadêmica do IPOG antes da publicação, com o registro na issue conforme as labels `compliance:mec`, `compliance:cfp`, `compliance:abrap` e `compliance:fbt` descritas em `CONTRIBUTING.md`. Número de credenciamento e-MEC nunca se estima: escreve-se "credenciamento e-MEC" sem número quando o número não veio do cliente. Conteúdo clínico sobre saúde mental carrega o peso adicional de YMYL, com autoria credenciada e disclaimer, porque leitor em sofrimento aplica em si o que lê.

### 14.3 Material didático segue o gênero de aula

Todo conteúdo que ensina alguém a fazer alguma coisa usa o esqueleto de "aula ou material de capacitação" da seção 3.2, mesmo quando a saída é uma página do portal e não uma aula. O problema chega como situação concreta com custo, o conceito é nomeado depois que o leitor já sabe por que precisa dele, e o exemplo resolvido do começo ao fim é o caso condutor da peça.

Os objetivos e os títulos de passo usam verbo de nível alto: aplicar, executar, diagnosticar, priorizar, comparar, avaliar, construir, projetar, recomendar, calcular. Os verbos recusados pela seção 3.2 (entender, conhecer, saber, identificar, listar, descrever) são os mesmos níveis 1 e 2 de Bloom que `content/BLOOM-KNOWLES.md` já desaconselhava em objetivo declarado, então as duas camadas apontam para o mesmo lugar e não há conflito a resolver. O fechamento é a síntese prática: o que a pessoa faz na segunda-feira, com qual artefato entregue e sob qual critério de pronto. O bloco "Exercícios para aplicar amanhã" prescrito em `prompts/PROMPT-REDACAO.md` é a implementação dessa síntese e continua obrigatório nas peças longas.

### 14.4 Nomenclatura das cinco modalidades

O escopo canônico do programa é "Pós-Graduações em Psicologia", com cinco modalidades: Especialização Lato Sensu (360 horas ou mais, reconhecida pelo MEC), MBA correlato à Psicologia, Mestrado Profissional, Especialização Clínica certificada por Conselhos (CFP, ABRAP, FBT, ABPp) e residências ou formações híbridas com supervisão. MBA é uma das cinco e nunca o nome do portfólio.

Peça que trata do programa abre declarando o escopo amplo; peça que trata de uma modalidade declara qual é, no título e no primeiro parágrafo. Tratar "MBA Online de Psicologia" como sinônimo do programa é o erro estrutural detectado em 12 de maio de 2026, que exigiu correção em 91 arquivos e encolhia o universo endereçável de psicólogos com registro ativo para a fração que procura MBA correlato. Ele segue catalogado como cliché banido na deny-list 4 de `content/CLICHES-PROIBIDOS.md`, e a taxonomia completa está em `prompts/KIT-PROMPTS-V0.md` e em `audits/benchmarking/README.md`.

A exigência de rótulo da seção 3.4 vale para modalidade, com a mesma lógica do teste do parágrafo solto da seção 13: o número viaja com o nome da modalidade dentro da mesma frase, porque "360 horas" recortado sem "Especialização Lato Sensu" chega ao leitor como se descrevesse o Mestrado Profissional, e é assim que a frase viaja quando alguém tira um print. No Schema.org, o `programType` acompanha a modalidade descrita no texto, sob pena de a página afirmar uma coisa para o leitor e outra para o motor.

### 14.5 Prova, marcadores e o que fazer quando falta

As quatro saídas da seção 2.2 valem na ordem, com tradução operacional. Pesquisar a origem significa chamar a Perplexity `sonar-pro` direto, e não pelo roteador do `geo-bridge.sh`, que segundo o anti-padrão 2 do `CLAUDE.md` devolve GPT-4o e confabula DOI. Reduzir a afirmação significa escrever o que a fonte sustenta, e não o que a página gostaria de dizer. Restringir o uso significa manter o argumento na peça técnica e tirá-lo da página de captação. Segurar a publicação é decisão legítima e barata comparada ao custo de um número errado sobre curso regulamentado.

Os marcadores canônicos são dois, `[FALTA EVIDÊNCIA: o que buscar]` e `[PREENCHER-HUMANO: o que falta]`, com teto de cinco abertos por documento. Este repositório carrega dois marcadores legados que se reconciliam quando o arquivo for tocado: `[VERIFICAR]`, com 36 ocorrências catalogadas em `docs/governance/verificar-followup-20260517.md`, e `[a calibrar na Fase 1]`, prescrito em `prompts/PROMPT-REVISAO.md`. Ambos viram `[FALTA EVIDÊNCIA]` quando a lacuna é pesquisável e `[PREENCHER-HUMANO]` quando o dado só existe dentro do IPOG. O catálogo legado segue como backlog e não autoriza marcador novo em arquivo novo.

O que só o cliente tem, e portanto nunca se estima: número de matrículas, taxa de conclusão, preço e condição de pagamento, número de credenciamento e-MEC, composição e titulação do corpo docente, resultado de egresso e decisão de produto ainda não comunicada. Estimar qualquer um deles contamina retroativamente tudo o que o programa já publicou sobre o cliente, que é o defeito mais caro previsto na seção 2.2.

### 14.6 Onde a doutrina governa a máquina

A doutrina governa o texto que nasce aqui apenas quando está dentro do prompt que roda, e por isso os arquivos de `prompts/` se dividem em dois grupos com tratamento oposto.

Recebem o bloco da doutrina os prompts que produzem ou julgam texto de leitura humana: `PROMPT-REDACAO.md`, que escreve a peça; `PROMPT-REVISAO.md`, que a reescreve; `PROMPT-ANALISE.md`, que a pontua; e `PROMPT-PESQUISA.md`, que recebe apenas a parte de prova da seção 2.2, por levantar a evidência que o redator vai usar. Não recebem nada os prompts que medem citação em motor generativo: `KIT-PROMPTS-V0.md`, `QUERIES-REGIONAIS-CANONICAS.md` e os prompts de `site/public/mcp/citation-prompts.json` disparados por `scripts/monitoring/run-prompts-weekly.mjs`. A razão é de medição, e não de estilo: instrução editorial dentro do prompt de coleta muda a resposta do motor e quebra a comparabilidade da série histórica de Mention Rate com as rodadas anteriores. `PROMPT-CLASSIFICACAO.md` também fica de fora, porque devolve metadata estruturada e nada que alguém leia.

O estado dos gates foi medido em 11 de agosto de 2026, e a medição contraria o que os documentos sugerem. Nenhum arquivo de regra editorial deste repositório é carregado por código: a busca por `CLICHES-PROIBIDOS`, `PADRAO-EDITORIAL`, `VOICE-GUIDE`, `ACENTUACAO-PT-BR` e `DIRETRIZ_EDITORIAL` em arquivos `.mjs`, `.py`, `.js`, `.ts`, `.yml`, `.json`, `.astro` e `.sh` devolve zero ocorrência. O Voice Guard citado em `PROMPT-REVISAO.md` e em `content/VOICE-GUIDE.md` é um cálculo que a LLM revisora faz dentro do próprio prompt, sem script que o execute aqui. O workflow `.github/workflows/quality.yml` roda o lychee com `fail: false` e o markdownlint com `continue-on-error: true`, ou seja, relata e nunca bloqueia. O `scripts/wiki/lint.py` funciona e verifica frontmatter, cross-link, órfão e stale claim do grafo, mas nenhum workflow o chama. Some tudo e sobra uma conclusão que precisa governar o comportamento de quem escreve: aqui a revisão da seção 13 é o único gate real, e aprovação de CI não diz nada sobre qualidade editorial.

A precedência, quando a regra local contradiz esta diretriz, resolve caso a caso com um critério só: a diretriz vence nas regras de forma que a v4 revogou de maneira explícita, e a regra local vence no que é específico do IPOG e do mercado regulado. O conflito encontrado nesta instalação foi o teto de linhas por parágrafo, que vivia em quatro lugares: `prompts/PROMPT-REDACAO.md` (parágrafo e síntese final), `prompts/PAPEIS-DE-COLETA.md` (critério de pronto do Redator), a dimensão `formatacao` de `prompts/PROMPT-ANALISE.md` e a paragrafação "3-5 linhas" da voz Brasil GEO em `content/VOICE-GUIDE.md`. É cota mecânica de ritmo, está revogado pela seção 4, item 8, e saiu dos quatro arquivos na instalação desta versão. Seguem valendo sem conflito as deny-lists de `content/CLICHES-PROIBIDOS.md`, os verbos e princípios de `content/BLOOM-KNOWLES.md`, o naming canônico de `CONTRIBUTING.md`, o compliance regulatório e os seis elementos de `content/PADRAO-EDITORIAL.md`, que são o gênero "artigo ou relatório" da seção 3.2 descrito com outro vocabulário.
